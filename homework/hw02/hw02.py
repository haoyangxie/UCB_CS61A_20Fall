...
def pingpong(n):
    ...
    def helper(index, direction,value):
        ...
        if index % 8 ==0 or num_eights(index):
            return helper(index+1, -1*direction, value - direction)
        else:
            return helper(index+1, direction, value + direction)
    return helper(1,1,1)
...
def make_anonymous_factorial():
    ...
    True
    """
    return (lambda f: lambda k: f(f, k))(lambda f, k: k if k == 1 or 0 else mul(k, f(f, sub(k, 1))))
