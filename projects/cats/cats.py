...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        ...
        for word in valid_words:
            diff[word] = diff_function(user_word, word, limit)
    if min(diff.values()) <= limit:
        result = [k for k, v in diff.items() if v == min(diff.values())]
        if len(result) == 0:
            return user_word
        else:
            return result[0]
...
def shifty_shifts(start, goal, limit):
    ...
    curr_char_not_equal = 1 if bool(start[0] != goal[0]) else 0
    return curr_char_not_equal + shifty_shifts(start[1:], goal[1:], limit - curr_char_not_equal)
    # END PROBLEM 6
